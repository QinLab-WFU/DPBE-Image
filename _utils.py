...
def save_checkpoint(args, best_checkpoint):
    ...
    if "memo" in best_checkpoint:
        ...
        if "epoch" in best_checkpoint.keys():
            ...
            torch.save(best_checkpoint, f"{args.save_dir}/{memo}.pkl")
    else:
...
def print_in_md(rst):
    ...
    datasets = sorted(
        ...
        key=lambda x: {"cifar": 1, "nuswide": 2, "flickr": 3, "coco": 4, "veri": 5, "vehicleid": 6}[x],
    )
...
if __name__ == "__main__":
    ...
    xxx = [
        "[dataset:veri][bits:256][best-epoch:189][best-mAP:0.585]",
        "[dataset:veri][bits:512][best-epoch:189][best-mAP:0.607]",
        "[dataset:veri][bits:1024][best-epoch:174][best-mAP:0.620]",
        "[dataset:veri][bits:2048][best-epoch:194][best-mAP:0.620]",
    ]
    rst = []
    for x in xxx:
        pattern = r"\[([a-zA-Z\-]+):([0-9\.a-z]+)\]"
        matches = re.findall(pattern, x)
        rst.append({k.lower().replace("-", "_").replace("bits", "hash_bit"): _conv(v) for k, v in matches})
    print_in_md(rst)
